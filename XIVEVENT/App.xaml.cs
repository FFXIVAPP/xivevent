...
namespace XIVEVENT {
    ...
    public partial class App : Application {
        ...
        private void ConfigureNLog() {
            string path = Path.Combine(Directory.GetCurrentDirectory(), "XIVEVENT.exe.nlog");
            StringReader stringReader;
            stringReader = File.Exists(path)
                               ? new StringReader(XElement.Load(path).ToString())
                               : new StringReader(ResourceHelper.LoadXML($"{Constants.AppPack}Resources/XIVEVENT.exe.nlog").ToString());
